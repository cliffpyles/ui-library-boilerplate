...
const normalizeBackgroundImage = (background, theme) => {
  ...
  if (background.image) {
    ...
    const normalized = normalizeBackground(theme.global.backgrounds?.[background], theme);
  ...
  if (backgroundImage.lastIndexOf("linear-gradient", 0) === 0) {
    ...
    result =
      backgroundImage.lastIndexOf("deg,") >= 0
        ...
        : backgroundImage.replace("linear-gradient(", `linear-gradient(${background.rotate}deg, `);
  } else {
    console.warn(
...
export const backgroundAndTextColors = (backgroundArg, textArg, theme) => {
  ...
  if (typeof background === "object") {
    ...
    if (background.color) {
      ...
      const opacity =
        background.opacity === true ? global.opacity.medium : global.opacity[background.opacity] || background.opacity;
...
export const backgroundStyle = (backgroundArg, theme, textColorArg) => {
  // for ComponentLibrary component, if the background isn't defined, don't set it
  if (backgroundArg === undefined) return undefined;

  const background = normalizeBackground(backgroundArg, theme);

  const [backgroundColor, textColor] = backgroundAndTextColors(background, textColorArg, theme);
    ...
    return css`
      ...
      ${!background.opacity
        ...
        &:before {
          ...
          ${backgroundStyles}
          opacity: ${background.opacity === true
...
export const getHoverIndicatorStyle = (hoverIndicator, theme) => {
  ...
  if (hoverIndicator === true || hoverIndicator === "background") {
    ...
    if (hoverIndicator.elevation || hoverIndicator.background) ({ elevation, background } = hoverIndicator);
  ...
  return css`
    ...
    ${elevation && `box-shadow: ${theme.global.elevation[theme.dark ? "dark" : "light"][elevation]};`}
  `;
};
