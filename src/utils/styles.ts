...
export const baseStyle = css`
  ...
  `}
  ${(props) => !props.plain && backgroundStyle(props.theme.baseBackground, props.theme)}
...
export const controlBorderStyle = css`
  border: ${(props) => props.theme.global.control.border.width} solid
    ${(props) => normalizeColor(props.theme.global.control.border.color || "border", props.theme)};
...
export const edgeStyle = (kind, data, responsive, responsiveBreakpoint, theme) => {
  ...
  const horizontalVerticalEqual = horizontal && vertical && horizontal === vertical;
  const allSidesEqual = top && bottom && left && right && ((top === bottom) === left) === right;
...
export const focusStyle = ({ forceOutline, justBorder, skipSvgChildren } = {}) => css`
  ${(props) =>
    !skipSvgChildren &&
    `
  > circle,
...
export const unfocusStyle = ({ forceOutline, justBorder, skipSvgChildren } = {}) => css`
  ${(props) =>
    !skipSvgChildren &&
    `
  > circle,
...
export const getInputPadBySide = (props, side) => {
  ...
  const pad = props.theme.global.input.padding[side] || props.theme.global.input.padding[orientation];
...
const placeholderColor = css`
  color: ${(props) => normalizeColor(props.theme.global.colors.placeholder, props.theme)};
`;
...
export const inputStyle = css`
  ...
  ${(props) =>
    `font-size: ${
      props.theme.global.input.font.size
        ? props.theme.text[props.theme.global.input.font.size]?.size || props.theme.global.input.font.size
        : "inherit"
    };`}
  font-family: inherit;
  border: none;
  ...
  color: inherit;
  width: 100%;
  ${(props) => props.theme.global.input.font.height && `line-height: ${props.theme.global.input.font.height};`}
  ${(props) =>
    props.theme.global.input.padding && typeof props.theme.global.input.padding !== "object"
      ? // On a breaking change release, this condition could be removed and
        ...
        `padding: ${
          parseMetricToNum(
            props.theme.global.edgeSize[props.theme.global.input.padding] || props.theme.global.input.padding
  ...
  ${(props) =>
    ...
    css`
      font-weight: ${props.theme.global.input.weight || props.theme.global.input.font.weight};
    `} margin: 0;
...
export const overflowStyle = (overflowProp) => {
  ...
  return css`
    ${overflowProp.horizontal && `overflow-x: ${overflowProp.horizontal};`} ${overflowProp.vertical &&
...
export const genericStyles = css`
  ...
  ${(props) =>
    props.margin &&
    props.theme.global &&
    edgeStyle("margin", props.margin, props.responsive, props.theme.global.edgeSize.responsiveBreakpoint, props.theme)}
`;

export const disabledStyle = (componentStyle) => css`
  opacity: ${(props) => componentStyle || props.theme.global.control.disabled.opacity};
  cursor: default;
`;
...
export const kindPartStyles = (obj, theme, colorValue) => {
  ...
  if (obj.padding || obj.pad) {
    // button uses `padding` but other components use ComponentLibrary `pad`
  ...
  if (obj.background)
    styles.push(
      backgroundStyle(
        ...
        theme,
        obj.color || (Object.prototype.hasOwnProperty.call(obj, "color") && obj.color === undefined ? false : undefined)
      )
    );
  else if (obj.color) styles.push(`color: ${normalizeColor(obj.color, theme)};`);
  if (obj.border) {
    if (obj.border.width)
      styles.push(css`
        border-style: solid;
    ...
    if (obj.border.color)
      styles.push(css`
        border-color: ${normalizeColor((!obj.background && colorValue) || obj.border.color || "border", theme)};
      `);
  ...
  if (obj.opacity) {
    const opacity =
      obj.opacity === true ? theme.global.opacity.medium : theme.global.opacity[obj.opacity] || obj.opacity;
...
export const roundStyle = (data, responsive, theme) => {
  ...
  if (typeof data === "object") {
    const size = ROUND_MAP[data.size] || theme.global.edgeSize[data.size || "medium"] || data.size;
    const responsiveSize =
      responsive && breakpoint && breakpoint.edgeSize[data.size] && (breakpoint.edgeSize[data.size] || data.size);
    if (data.corner === "top") {
      styles.push(css`
    ...
    `);
    const responsiveSize = responsive && breakpoint && breakpoint.edgeSize[size];
    if (responsiveSize) {
      styles.push(
        breakpointStyle(
          breakpoint,
...
export const alignContentStyle = css`
  align-content: ${(props) => ALIGN_CONTENT_MAP[props.alignContent] ?? props.alignContent};
`;
...
const widthStringStyle = (width, theme) => css`
  width: ${getSize(theme, width)};
`;

export const widthStyle = (width, theme) =>
  typeof width === "object" ? widthObjectStyle(width, theme) : widthStringStyle(width, theme);
...
const heightStringStyle = (height, theme) => css`
  height: ${getSize(theme, height)};
`;

export const heightStyle = (height, theme) =>
  typeof height === "object" ? heightObjectStyle(height, theme) : heightStringStyle(height, theme);
