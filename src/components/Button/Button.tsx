...
const getIconColor = (paths = [], theme, colorProp, kind) => {
  ...
  while (index >= 0 && !result[1]) {
    ...
    if (obj) {
      ...
      const background = colorProp && obj.background && obj.background.color ? colorProp : obj.background;

      // if theme object explicitly sets the color to undefined, pass false
      // to indicate that the theme doesn't want any text color
      const objColor =
        obj.color ||
        (Object.prototype.hasOwnProperty.call(obj, "color") && obj.color === undefined ? false : undefined);
      ...
      if (!color)
        color =
          colorProp && (!obj.background || !obj.background.color) && (!obj.border || !obj.border.color)
            ? colorProp
            : objColor;
...
const getPropertyColor = (property, paths = [], theme, kind, primary) => {
  ...
  if (kind) {
    ...
    result =
      ...
      (theme && theme.button && theme.button[property] && theme.button[property].color);
  }
  return result;
};
  ...
  (
    ...
    const isDarkBackground = () => {
      const backgroundColor = normalizeBackground(
        normalizeColor(
          color || (theme.button.primary && theme.button.primary.color) || theme.global.colors.control || "brand",
          theme
        ),
        theme
      );
    ...
    const kindIcon =
      (hover && getKindIcon(themePaths?.hover, theme, kind)) || getKindIcon(themePaths?.base, theme, kind);
    ...
    if (icon && !icon.props.color) {
      if (kind) {
        if (!plain) {
          ...
          const iconColor =
            (hover && getIconColor(themePaths.hover, theme)) || getIconColor(themePaths.base, theme, color, kind);
    ...
    const background = getPropertyColor("background", themePaths && themePaths.base, theme, kind, primary);
    const border = getPropertyColor("border", themePaths && themePaths.base, theme, kind, primary);
    ...
    const innerBadge =
      theme.button?.badge?.align !== "container" && ((!background && !border) || (!kind && icon && !label));
    ...
    if (busy || success) {
      ...
      contents = (
        ...
        <RelativeBox flex={false}>
          ...
          {success && (
            <Box style={{ position: "absolute" }} fill alignContent="center" justify="center">
              <GrowCheckmark color={animationColor} aria-hidden />
            </Box>
          )}
          <StyledBusyContents animating={busy || success}>{contents}</StyledBusyContents>
        </RelativeBox>
      );
    }
    ...
    if (badgeProp && !innerBadge) {
      styledButtonResult = <Badge content={badgeProp}>{styledButtonResult}</Badge>;
    }
