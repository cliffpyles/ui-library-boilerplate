...
const TestInput = ({ theme, containerTarget, message = "this is a test", ...rest }: TestInputProps) => {
  ...
  return (
    <ComponentLibrary theme={theme} containerTarget={containerTarget}>
...
const TestButton = ({ theme, containerTarget, message = "this is a test", ...rest }: TestButtonProps) => {
  const [showDrop, setShowDrop] = useState<boolean>(false);
  ...
  if (showDrop) {
    drop = (
      <Drop id="drop-node" inline target={buttonRef.current || undefined} {...rest}>
        {message}
      </Drop>
    );
  }
...
describe.skip("Drop", () => {
  ...
  test("stretch = align", () => {
    const message = "test test test test test test test test test test test test test test";
  ...
  test("should render correct margin depending on value of align", () => {
    ...
    const results = alignPositions.map((alignPosition) => {
      ...
      const result = render(<TestInput theme={customMarginTheme} align={alignPosition} />);
      return result;
    });
...
test("custom containerTarget", () => {
  ...
  try {
    const { getByTestId } = render(<TestInput data-testid="drop" containerTarget={target} />);
    const actualRoot = getByTestId("drop")?.parentNode?.parentNode?.parentNode;
