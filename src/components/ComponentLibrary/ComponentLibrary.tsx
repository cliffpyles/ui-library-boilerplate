...
import { StyledComponentLibrary } from "./StyledComponentLibrary";
...
import { ComponentLibraryPropTypes } from "./propTypes";
...
const ComponentLibrary = forwardRef((props, ref) => {
  const {
    children,
    full,
  ...
  const messages = useMemo(() => {
    ...
    const nextMessages = deepMerge(defaultMessages, messagesProp?.messages || {});
    return {
      messages: nextMessages,
      format: (opts) => {
        const message = messagesProp?.format && messagesProp.format(opts);
        return typeof message !== "undefined" ? message : format(opts, nextMessages);
      },
    };
  }, [messagesProp]);
  ...
  const responsive = stateResponsive || deviceResponsive(userAgent, theme) || theme.global.deviceBreakpoints.tablet;

  const componentLibraryRef = useForwardedRef(ref);

  useEffect(() => {
    if (componentLibraryRef.current) setRoots([componentLibraryRef.current]);
  }, [componentLibraryRef]);
              ...
              <MessageContext.Provider value={messages}>
                <AnalyticsProvider onAnalytics={onAnalytics}>
                  <StyleSheetManager shouldForwardProp={shouldForwardProp}>
                    <StyledComponentLibrary full={full} {...rest} ref={componentLibraryRef}>
                      {children}
...
ComponentLibrary.propTypes = ComponentLibraryPropTypes;
